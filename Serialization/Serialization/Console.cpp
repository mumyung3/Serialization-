...
void Buffer_Flip(void) {
	...
	for (int i = 0; i < dfSCREEN_HEIGHT; i++) {
		cs_MoveCursor(0, i);
		wprintf(L"%-*s", dfSCREEN_WIDTH - 1, szScreenBuffer[i]);
...
void DrawLayout(void) {
	...
	swprintf_s(szScreenBuffer[3], sizeof(szScreenBuffer[3]) / sizeof(wchar_t), L"%s %s",
		L"║ 연산(함수명):", operatorName
	);
	swprintf_s(szScreenBuffer[4], sizeof(szScreenBuffer[4]) / sizeof(wchar_t), L"%s %s",
		L"║ 타입:", typeName
	);
	swprintf_s(szScreenBuffer[5], sizeof(szScreenBuffer[5]) / sizeof(wchar_t), L"%s %d",
		L"║ 크기:", compareSize
	);
	swprintf_s(szScreenBuffer[6], sizeof(szScreenBuffer[6]) / sizeof(wchar_t), L"%s %s",
		L"║ 테스트 값:", testValStr
	);
	swprintf_s(szScreenBuffer[7], sizeof(szScreenBuffer[7]) / sizeof(wchar_t), L"%s %s",
		L"║ 논리 값:", logicValStr
	);
	swprintf_s(szScreenBuffer[8], sizeof(szScreenBuffer[8]) / sizeof(wchar_t), L"%s %d/%d",
		L"║  PASS/FAIL:", cntPass, cntFail
	);
	swprintf_s(szScreenBuffer[9], sizeof(szScreenBuffer[9]) / sizeof(wchar_t), L"%s",
		L"╠══════════════════════════════════════════════╣"
	);
	swprintf_s(szScreenBuffer[10], sizeof(szScreenBuffer[10]) / sizeof(wchar_t), L"%s %d",
		L"║ 상태:", bIsPass
	);
